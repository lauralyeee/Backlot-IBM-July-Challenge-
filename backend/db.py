"""
Turso (libSQL) database layer for Backlot.
Tables: worlds, assets.

Vercel's serverless functions have a read-only filesystem and no shared
state between instances -- a local worldbuilding.db file would mean every
request could silently see different or empty data. Turso is the required
storage layer once this runs on Vercel, not an optional swap.

Connection target:
  - TURSO_DATABASE_URL + TURSO_AUTH_TOKEN set -> connects to Turso Cloud
    over HTTP (libsql "remote access" mode). This is what production/Vercel
    uses, and it's also what local dev should use once a Turso database
    exists, so the same code path is exercised everywhere.
  - Neither set -> falls back to a local file at DB_PATH (still via the
    libsql engine, which is a SQLite-compatible fork) purely so the app can
    still boot without Turso credentials during early local development.
    This fallback is never valid on Vercel -- the filesystem there won't
    persist writes across invocations.
"""
...
def _connect():
    if TURSO_DATABASE_URL:
        ...
        # Local-dev-only fallback -- see module docstring. Not valid on Vercel.
        conn = libsql.connect(DB_PATH)
    # Turso Cloud manages WAL/durability server-side, so journal_mode=WAL is
    # dropped (it was sqlite3-file-specific and doesn't apply over HTTP).
    conn.execute("PRAGMA foreign_keys=ON")
    return conn


# The libsql Connection has no `row_factory` attribute (unlike sqlite3), so
# rows come back as plain tuples. These two helpers dictify a cursor's
# result using cursor.description, so every _row_to_*() function below can
# keep using row["col"] access unchanged -- call sites just need to hold
# onto the cursor instead of chaining .fetchone()/.fetchall() directly off
# conn.execute(...).
def _one(cur):
    row = cur.fetchone()
    if row is None:
...
def init_db():
    with _connect() as conn:
        ...
        existing_cols = {row["name"] for row in _all(conn.execute("PRAGMA table_info(assets)"))}
        ...
        world_cols = {row["name"] for row in _all(conn.execute("PRAGMA table_info(worlds)"))}
    ...
    with _connect() as conn:
        conn.execute(
            ...
            (
                ...
                json.dumps(data["eras"]),
                json.dumps(data.get("eraNotes", {})),
                json.dumps(data["ideas"]),
                json.dumps(data.get("dialects", {})),
...
def get_world(world_id: str) -> dict | None:
    with _connect() as conn:
        row = _one(conn.execute("SELECT * FROM worlds WHERE id = ?", (world_id,)))
...
def update_world(world_id: str, data: dict) -> dict | None:
    ...
    for key, col in mapping.items():
        if key in data:
            fields.append(f"{col} = ?")
            values.append(json.dumps(data[key]) if key in json_fields else data[key])
    if not fields:
        return get_world(world_id)
    values.append(world_id)
    with _connect() as conn:
        conn.execute(f"UPDATE worlds SET {', '.join(fields)} WHERE id = ?", tuple(values))
        conn.commit()
    return get_world(world_id)
...
def get_asset(asset_id: int) -> dict | None:
    with _connect() as conn:
        row = _one(conn.execute("SELECT * FROM assets WHERE id = ?", (asset_id,)))
...
def update_asset(asset_id: int, data: dict) -> dict:
    ...
    with _connect() as conn:
        conn.execute(f"UPDATE assets SET {', '.join(fields)} WHERE id = ?", tuple(values))
        conn.commit()
    return get_asset(asset_id)
    ...
    with _connect() as conn:
        conn.execute(
            """INSERT INTO documents (id, world_id, title, raw_text, created_at)
               VALUES (?, ?, ?, ?, ?)""",
            (data["id"], data["world_id"], data["title"], data["raw_text"], data["created_at"]),
        )
        conn.commit()
...
def get_document(document_id: str) -> dict | None:
    with _connect() as conn:
        row = _one(conn.execute("SELECT * FROM documents WHERE id = ?", (document_id,)))
...
def get_relationship(relationship_id: str) -> dict | None:
    with _connect() as conn:
        row = _one(conn.execute("SELECT * FROM relationships WHERE id = ?", (relationship_id,)))
...
def get_export_version(version_id: str) -> dict | None:
    with _connect() as conn:
        row = _one(conn.execute("SELECT * FROM export_versions WHERE id = ?", (version_id,)))
